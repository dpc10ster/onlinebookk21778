PlotRsmProp <- function(mu, lambdaP, nuP, nLesDistr, c, da, fpf, tpf, i, j){
  ...
  fitPlot <- ggplot() + geom_line(mapping = aes(x = FPF, y = TPF, color = Model), data = plotCurve, size = 1) + 
    geom_line(data = dashedSm, aes(x = FPF, y = TPF, color = Model), linetype = 3, size = 1) + scale_color_manual(values = c("blue", "red")) +
    geom_point(mapping = aes(x = FPF, y = TPF), data = plotOp, size = 3) +
    theme(legend.position = "none") + 
    labs(title = ij) +
    scale_x_continuous(expand = c(0, 0)) + 
    scale_y_continuous(expand = c(0, 0))
  #cat("Num SM AUC =", trapz(c(rev(FPFSm), 1), c(rev(TPFSm), 1)), "Num PROP AUC =", trapz(rev(FPFProp), rev(TPFProp)), "\n")
  return(fitPlot)
}