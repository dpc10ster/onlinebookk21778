FitOldROCCurve <- function(simuData, selectedMod, selectedRdr, nLesDistr){ 
  ...
  for (i in 1:I){
    for (j in 1:J){
      ...
      if (FPF[length(FPF)] == 1){
        lambdaP <- -2 * log(1 - 0.99)
      }else if (FPF[length(FPF)] == 1){
        lambdaP <- -2 * log(1 - 0.01)
      }else{
        lambdaP <- -2 * log(1 - FPF[length(FPF) - 1])
      }
...
# NllROCNew <- function (lambdaPFwd, nuPFwd, sumNLFit, fb, tb, K1, K2, nLesDistr, 
...
#   zeta <- unlist(mget(allParameters[zetaPos]))
#   zeta <- InverseZetas(zeta)
#   mu <- optimize(ChisqMuROC, c(minMu, maxMu), zeta = zeta, 
#                  fb = fb, tb = tb, K1 = K1, K2 = K2, lambdaP = lambdaP, 
#                  nuP = nuP, nLesDistr = nLesDistr)$minimum
#   FPF <- xROC(zeta, lambdaP)
#   TPF <- yROC(zeta, mu, lambdaP, nuP, nLesDistr)
#   FPFTerms <- c(1, FPF) - c(FPF, 0)
#   TPFTerms <- c(1, TPF) - c(TPF, 0)
