...
{  
  ...
  for (lineNumber in 1:length(lines)){
    ...
    if (length(grep("PROCEDURE DOES NOT CONVERGE", temp)) != 0) {
      return(-1)
    }
    
    if (length(grep("--- program is running; please wait ---", temp) != 0) && 
        (lineNumber == length(lines))) {
      return(-1)
    }    
    
    if (length(grep("LOSS OF SIGNIFICANCE", temp) != 0) && 
        (lineNumber == length(lines))) {
      return(-1)
    }
  }
  ...
  ptm <- proc.time()
  while(1){
    if ((as.numeric(proc.time() - ptm))[3] > 120) return (-1) # CORROC2 is hanging up; quit with error
    temp <- lines[lineNumber]
  ...
  ptm <- proc.time()
  while(1){
    if ((as.numeric(proc.time() - ptm))[3] > 120) return (-1)
    temp <- lines[lineNumber]
