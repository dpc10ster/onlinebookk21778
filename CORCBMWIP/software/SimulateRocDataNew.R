    ...
    for (i1 in 1:I) {
      for (j1 in 1:J) {
        ...
        for (i2 in 1:I) {        
          for (j2 in 1:J) {
            if (t == 1){
              SigmaA[t, countRow, countCol] <- ChiNew[i1, i2, j1, j2, 2] * ChiNew[i1, i2, j1, j2, 4] * ChiNew[i1, i2, j1, j2, 5]
            }else{
              SigmaA[t, countRow, countCol] <- 1 * 1 * ChiNew[i1, i2, j1, j2, 6]
            }
            countRow <- countRow + 1
          }
        }
  ...
  for (t in 1:2){
    ...
    for (i1 in 1:I) {
      for (j1 in 1:J) {
        ...
        for (i2 in 1:I) {        
          for (j2 in 1:J) {
            cat(t, i1, j1, i2, j2, SigmaA[t, countRow, countCol],"\n")
          }
          countRow <- countRow + 1
        }
      }
