...
for (i in 1:I){
  for (j in 1:J) {    
    msTR <- msTR + 
      (FOM[i,j] - mean(FOM[i,]) - mean(FOM[,j]) + mean(FOM))^2    
  }  
}
msTR <- msTR / ((I - 1)*(J - 1))
...
cat("F statistic is ", F_ORH, "and critical value of F is ", FCrit, "\n")  
pValue <- 1 - pf(F_ORH, ndf, ddfH)
cat("pvalue = ", pValue, "\n")
...
for (i in 1 : nDiffs) {
  ...
  CI_DIFF_FOM_RRRC[i,2] <- 
    qt(alpha/2,df = ddfH)*std_DIFF_FOM_RRRC + trtDiff[i]
  CI_DIFF_FOM_RRRC[i,3] <- 
    qt(1-alpha/2,df = ddfH)*std_DIFF_FOM_RRRC + trtDiff[i]
  cat("For pairing", strDiff[i], 
      "\nmean diff is ", CI_DIFF_FOM_RRRC[i,1], 
      "\nand 95% CI is ", CI_DIFF_FOM_RRRC[i,2], 
      CI_DIFF_FOM_RRRC[i,3], "\n")
}
...
for (i in 1 : nDiffs) {
  ...
  CI_DIFF_FOM_FRRC[i,2] <- 
    qt(alpha/2,df = ddf)*std_DIFF_FOM_FRRC + trtDiff[i]
  CI_DIFF_FOM_FRRC[i,3] <- 
    qt(1-alpha/2,df = ddf)*std_DIFF_FOM_FRRC + trtDiff[i]
  cat("For pairing", strDiff[i], 
      "\nmean diff is ", CI_DIFF_FOM_FRRC[i,1], 
      "\nand 95% CI is ", CI_DIFF_FOM_FRRC[i,2], 
      CI_DIFF_FOM_FRRC[i,3], "\n")
}
...
for (i in 1 : nDiffs) {
  ...
  CI_DIFF_FOM_RRFC[i,2] <- 
    qt(alpha/2,df = ddf)*std_DIFF_FOM_RRFC + trtDiff[i]
  CI_DIFF_FOM_RRFC[i,3] <- 
    qt(1-alpha/2,df = ddf)*std_DIFF_FOM_RRFC + trtDiff[i]
  cat("For pairing", strDiff[i], 
      "\nmean diff is ", CI_DIFF_FOM_RRFC[i,1], "\nand 95% CI is ", 
      CI_DIFF_FOM_RRFC[i,2], CI_DIFF_FOM_RRFC[i,3], "\n")
}

plotM <- list(1, 2, 3, 4)
plotR <- list(c(1:5), c(1:5), c(1:5), c(1:5))
plot <- PlotEmpiricalOperatingCharacteristics(
  dataset = rocData, trts = plotM, rdrs = plotR)
p <- plot$Plot +
  ...
  theme(axis.title.y = element_text(size = 25,face="bold"),
        ...
        legend.text = element_text(size = 15, face = "bold"),legend.key.size = unit(2, "lines")) +
