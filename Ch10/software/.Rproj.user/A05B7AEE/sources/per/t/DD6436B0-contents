SingleTreatmentAnalysis <- function(rocData1T, theta0, covEstMthd = "Jackknife", alpha = 0.05){
  fomArray <- UtilFigureOfMerit(rocData1T, FOM = "Wilcoxon")
  J <- length(fomArray)
  NL <- rocData1T$NL
  LL <- rocData1T$LL
  K <- dim(NL)[3]
  ...
  zijk2 <- LL
  dim(zijk2) <- c(1, J, K2)
  if (covEstMthd == "Jackknife"){
    cov2 <- CovJk(zijk1, zijk2)$Cov2  
  }else if (covEstMthd == "DeLong"){
