rm(list = ls()) # mainCadVsRadCalibValidate.R
library(RJafroc)
library(doParallel)
library(doRNG)
library(foreach)
...
f <- 9;fileName <- fileNames[f] #NICO
retFileName <- paste0("saveRetRoc", fileName) 
dataset <- get(sprintf("dataset%02d", f))
retCalibSimParms <- calibrateMutivariateSimulator(dataset)
...
for (q in 1:length(simuK1Arr)){
  ...
  for(s in 1:S) {
    index <- index + 1
    # muAlpha <- rmvnorm(J, mean = params1[1:2], sigma = covMatr1)
    simuMu <- rep(0, J)
    simuAlpha <- rep(0, J)
    ...
    # simuMu[1] <- mean(simuMu[2:J])
    # simuAlpha[1] <- mean(simuAlpha[2:J])
    ...
    if (binning){
      simuData <- DfBinDataset(simuData)
    }
    ...
    retSimu[[index]] <- c(ret$cov2*((K1+K2)/200), ret$var*((K1+K2)/200), ret$pVal)# !dpc
  }
  options(digits = 6)
  ...
  if (retOrh$cov2 >= lower && retOrh$cov2 <= upper){
  ...
  if (retOrh$varEps >= lower && retOrh$varEps <= upper){
