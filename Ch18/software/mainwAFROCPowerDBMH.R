...
sysAnalFileName <- system.file("ANALYZED/RSM5", retFileName, package = "RJafroc")
...
nLesDistr <- allResults[[1]]$lesionDistribution
...
JTest <- 5;KTest <- 100
varCompROC <- StSignificanceTesting(frocData, fom = "HrAuc", method = "DBMH", option = "RRRC")$varComp
...
for (i in 1:length(effectSizeROC)) {
  ...
  powerROC[i] <- SsPowerGivenJK(JTest, KTest, alpha = 0.05, effectSize = effectSizeROC[i], option = "RRRC",
                                method = "DBMH", varYTR = varYTR, varYTC = varYTC, varYEps = varYEps)$powerRRRC
