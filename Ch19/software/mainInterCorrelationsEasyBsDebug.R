# mainInterCorrelationsEasyBsDebug.R 
...
# boostrap standard (non-cluster) code follows 
names(clusterParms) <- fileNames
B <- 20;bootStrapResults <- array(dim = c(B,9))
for (b in 1:B) {
  ...
  bootStrapResults[b,] <- c(mean(AucPro1),mean(AucCbm1),mean(AucRsm1),
