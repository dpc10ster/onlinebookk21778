PlotBMErrBar <- function(a, b, rocDataTable, rowIndx){
  ...
  fitPlot <- ggplot() + geom_line(mapping = aes(x = FPF, y = TPF), data = plotBM, size = 2) + 
    geom_point(mapping = aes(x = FPF, y = TPF), data = plotOpPnts, size = 5) + 
    labs(title = rowIndx) + theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold")) + 
